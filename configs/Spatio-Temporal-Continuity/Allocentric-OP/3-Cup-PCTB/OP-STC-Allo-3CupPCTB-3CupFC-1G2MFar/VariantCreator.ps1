$MasterYAML = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*.yml'

$copyto = $MasterYAML.FullName -replace "-Grey-","-DarkGreen-"
Copy-Item $MasterYAML.FullName $copyto
$DarkGreenVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-DarkGreen-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BeigeVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-Beige-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$OffwhiteVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-Off-white-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$DarkBlueVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-DarkBlue-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BrownVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-Brown-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$RandomVariant = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*-Random-*.yml'
...
$ColourVariantList = Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*.yml'
foreach ($row in $ColourVariantList){
$copyto = $row.FullName -replace "Opaque","OpLava"
Copy-Item $row.FullName $copyto}

Get-ChildItem '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*OpLava*.yml' | ForEach {
(Get-Content $_ | ForEach {
$_.Replace("- !Vector3 {x: 12, y: 2.1, z: 4}", "- !Vector3 {x: 6, y: 2.1, z: 4}") }) |
Set-Content $_
}

Add-Content -Path '.\OP-STC-Allo1-3CupPCTB-3CupFC-1goalMfar\*OpLava*.yml' -Value "    - !Item
      name: DeathZone
      positions:
