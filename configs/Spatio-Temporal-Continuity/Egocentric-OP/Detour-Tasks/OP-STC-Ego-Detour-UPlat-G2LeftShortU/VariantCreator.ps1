$MasterYAML = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*.yml'

$copyto = $MasterYAML.FullName -replace "-Grey-","-DarkGreen-"
Copy-Item $MasterYAML.FullName $copyto
$DarkGreenVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-DarkGreen-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BeigeVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-Beige-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$OffwhiteVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-Off-white-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$DarkBlueVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-DarkBlue-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BrownVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-Brown-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$RandomVariant = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-Random-*.yml'
...
$ColourVariantList = Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*.yml'
foreach ($row in $ColourVariantList){
$copyto = $row.FullName -replace "-0-","-180-"
Copy-Item $row.FullName $copyto}

Get-ChildItem '.\OP-STC-Ego-Detour-UPlat-LeftShortU\*-180-*.yml' | ForEach {
(Get-Content $_ | ForEach { $_.Replace("rotations: [330] #Agent Rotation Parameter", "rotations: [150] #Agent Rotation Parameter") }) |
Set-Content $_
}
