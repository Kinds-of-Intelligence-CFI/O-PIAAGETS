$MasterYAML = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*.yml'

$copyto = $MasterYAML.FullName -replace "-Grey-","-DarkGreen-"
Copy-Item $MasterYAML.FullName $copyto
$DarkGreenVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-DarkGreen-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BeigeVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-Beige-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$OffwhiteVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-Off-white-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$DarkBlueVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-DarkBlue-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$BrownVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-Brown-*.yml'
...
Copy-Item $MasterYAML.FullName $copyto
$RandomVariant = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-Random-*.yml'
...
$ColourVariantList = Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*.yml'
foreach ($row in $ColourVariantList){
$copyto = $row.FullName -replace "-0-","-180-"
Copy-Item $row.FullName $copyto}

Get-ChildItem '.\OP-STC-Ego-Detour-Ramp-LeftDiscret\*-180-*.yml' | ForEach {
(Get-Content $_ | ForEach { $_.Replace("rotations: [45] #Agent Rotation Parameter", "rotations: [225] #Agent Rotation Parameter") }) |
Set-Content $_
}
